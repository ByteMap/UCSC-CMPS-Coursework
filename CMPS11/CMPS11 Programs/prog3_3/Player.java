...
public class Player {
  ...
  //player can smell the mushroom
   public String smellMushroom() { 
    return "Something smells good!"; 
  }
   
 //player can interact with Charlie!
   public String tickleCharlie() {
    return "Charlie: That tickles!";
  }
   
 //checks to see if player's pack contains a certain item 
   public boolean containsItem(String thing) {
     boolean contains = false;
     for(int i = 0; i<pack.size(); i++) {
       if(pack.get(i).name().equals(thing)){
         contains = true;
       }
     }
     return contains;
 ...
 //prints message after slaying frog  
   public String slay() {
     return "You have slain the evil frog.";
   }
   
//becomes true after player has slain frog
   public boolean hasSlainFrog(boolean check) {
     return check;
   }
 
   public String winningMessage() {
     return "You got all the mushrooms to Charlie in time for his soup.\n" + 
       "Thanks for playing!";
   }
