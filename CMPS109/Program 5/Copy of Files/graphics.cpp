...
void window::display() {
   ...
   //Set a counter to count the indices of the objects in the
   //list.
   size_t counter = 0;
   //Traverse through all of the objects
   for (auto& object: window::objects) {
      //Set the object number to the counter.
      object.get_num() = counter;
      //If the selected_obj is equal to the counter:
      //(i.e. the f-key corressponds to the object in the list)
      //Then select the object to flag for drawing a border along with
      //drawing the shape.
      if(window::selected_obj == counter++) {
         //cout << "Selected " << window::selected_obj << endl;
         object.select();
         object.get_bord_thickness() = window::border_thickness;
         object.get_bord_color() = window::border_color;
      }
      //Else readjust the object position if the position will be out of
      //the window bounds
      if(object.get_center().xpos < 0) { 
         object.get_center().xpos = window::width; 
      } 
      if(object.get_center().xpos > window::width) { 
         object.get_center().xpos = 0;
      }
      if(object.get_center().ypos < 0) {
         object.get_center().ypos = window::height; 
      }
      if(object.get_center().ypos > window::height) {
         object.get_center().ypos = 0; 
      }
      //And draw the shape.
      object.draw();
      //Draw the number if the object number (counter) is 0-9
      //object.display_num(counter);
      object.deselect();
   }
   mus.draw();
   glutSwapBuffers();
...
void window::select_object(size_t object) {
   if(object < window::objects.size()) {window::selected_obj = object;}
}

void window::move_selected_object(GLfloat xpos, GLfloat ypos) {
   objects[selected_obj].move(xpos, ypos);
}
...
void window::keyboard (GLubyte key, int x, int y) {
   ...
   switch (key) {
      case 'Q': case 'q': case ESC:
         ...
         break;
      case 'H': case 'h':
         move_selected_object (pixels_to_move * -1, 0);
         break;
      case 'J': case 'j':
         move_selected_object (0, pixels_to_move * -1);
         break;
      case 'K': case 'k':
         move_selected_object (0, pixels_to_move);
         break;
      case 'L': case 'l':
         move_selected_object (pixels_to_move, 0);
         break;
...
void window::special (int key, int x, int y) {
   ...
   window::mus.set (x, y);
   switch (key) {
      case GLUT_KEY_LEFT: move_selected_object (pixels_to_move * -1, 0);
                           break;
      case GLUT_KEY_DOWN: move_selected_object (0, pixels_to_move * -1);
                           break;
      case GLUT_KEY_UP: move_selected_object (0, pixels_to_move); break;
      case GLUT_KEY_RIGHT: move_selected_object (pixels_to_move, 0);
                           break;
