...
equilateral::equilateral (GLfloat width): 
         triangle({0, 0}, {width/2, (width * sqrt(3))/2}, {width, 0}) {
   DEBUGF ('c', this);
}
...
void text::draw (const vertex& center, const rgbcolor& color) const {
   ...
   auto text = reinterpret_cast<const Glubyte*> (window.text.cstr());
   glColor3ubv(color.ubvec);
   glRasterPos2f(center.xpos, center.ypos);
   glutBitmapString(glut_bitmap_font, text);

   if(his::get_num() < 10) { draw_number(); }
}
...
void ellipse::draw (const vertex& center, const rgbcolor& color) const {
   ...
   if(object::get_num() < 10) { draw_number(); }

   if(object::select_status()) {
...
void polygon::draw (const vertex& center, const rgbcolor& color) const {
   ...
   if(this::get_num() < 10) { draw_number(); }
