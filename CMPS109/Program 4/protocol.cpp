...
void recv_packet (base_socket& socket, void* buffer, size_t bufsize) {
   ...
   do {
      ...
      if (nbytes == 0) throw socket_error (to_string (socket)
                                           + " is closed");
      //set the next buffer position to read into: determined by the 
      //number of bytes that was already read into the current buffer
      //position
      bufptr += nbytes;
      //nbytes has been already read into the buffer, so reduce nbytes
      //from ntorev so that the buffer will read less in the next loop.
      ntorecv -= nbytes;
   }while (ntorecv > 0);
}
