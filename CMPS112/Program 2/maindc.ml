...
let print_number number = 
    let length = String.length (string_of_bigint number) in
    let num = (string_of_bigint number)
        in let rec print_rec index  = match (length - index) < 69
           with
           | true   -> printf "%s\n%!" (String.sub num index 
                                       (length - index))
           | false  ->  (printf "%s\\\n%!" (String.sub num index 69);
                        print_rec (index + 69))
    in print_rec 0
...
let load symbol stack =
    let entry = Array.get symbol_table symbol
                in match entry with
                   | false, _    -> 
                              printf "register '%d' is empty\n%!" symbol
                   | true, value -> push value stack

let store symbol value = 
    Array.set symbol_table symbol (true, value)
